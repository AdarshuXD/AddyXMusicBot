...
from AddyXMusic.misc import HAPP, SUDOERS, XCB
from AddyXMusic.utils.database import (
    get_active_chats,
    remove_active_chat,
    remove_active_video_chat,
)
from AddyXMusic.utils.decorators.language import language
from AddyXMusic.utils.pastebin import Addybin
...
<u>Usage:</u>
Total Used: `{AppHours}`**ʜ**  `{AppMinutes}`**M**  [`{AppPercentage}`**%**]
...
async def update_(client, message, _):
    ...
    if await is_heroku():
        try:
            ...
            for x in served_chats:
                try:
                    await app.send_message(
                        x,
                        f"{config.MUSIC_BOT_NAME} Updating The Bot. Sorry For The Issues.\n\nYou Can Start Playing Again After 15-20 Seconds.",
                    )
                    ...
                    pass
            await response.edit(
                f"{nrs.text}\n\nBot Updated Successfully ! Now Wait For Few Minutes Until The Bot Restarts And Push Changes!"
            )
            os.system(
            ...
            await response.edit(
                f"{nrs.text}\n\nSomething Went Wrong When Tried To Restart The Music Bkt, Please Check Logs To Know What's Wrong."
            )
        ...
        for x in served_chats:
            try:
                await app.send_message(
                    x,
                    f"{config.MUSIC_BOT_NAME} Has Just Restarted Himself For Updating The Bot. Sorry For The Issue.\n\nYou Can Start Playing Again After 15-20 Seconds.",
                )
                await remove_active_chat(x)
                await remove_active_video_chat(x)
            except Exception:
                pass
        await response.edit(
            f"{nrs.text}\n\nBot Updated Successfully ! Now Wait For Few Minutes Until The Bot Restarts And Puch Changes !"
        )
