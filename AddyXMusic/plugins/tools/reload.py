...
from AddyXMusic.utils.database import get_authuser_names, get_cmode
from AddyXMusic.utils.decorators import ActualAdminCB, AdminActual, language
from AddyXMusic.utils.formatters import alpha_to_int
...
async def restartbot(client, message: Message, _):
    mystic = await message.reply_text(
        f"Please Wait, Getting Restart {MUSIC_BOT_NAME} For Your Chat."
    )
    await asyncio.sleep(1)
    try:
        db[message.chat.id] = []
        await Addy.stop_stream(message.chat.id)
    except:
        pass
    ...
    return await mystic.edit_text(
        "Successfully Restated {MUSIC_BOT_NAME} For Your Chat, Now You Can Play Music In Your Group Again..."
    )
...
async def stop_download(client, CallbackQuery: CallbackQuery, _):
    ...
    await CallbackQuery.answer("Failed To Recognise The On Going Task.", show_alert=True)
