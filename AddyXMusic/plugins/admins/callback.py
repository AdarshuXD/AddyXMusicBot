...
from AddyXMusic import YouTube, app
from AddyXMusic.core.call import Addy
from AddyXMusic.misc import SUDOERS, db
...
from AddyXMusic.utils.formatters import seconds_to_min
from AddyXMusic.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
from AddyXMusic.utils.stream.autoclear import auto_clean
from AddyXMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Addy.pause_stream(chat_id)
        ...
        await Addy.resume_stream(chat_id)
        ...
        await Addy.mute_stream(chat_id)
        ...
        await Addy.unmute_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Addy.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Addy.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Addy.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_9"])
            ...
            try:
                await Addy.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Addy.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
